...
//////////////////////////// SYNC FUNCTIONS //////////////////////////
     ...
     String shoplisthashbrown(){
        String dbString= "";
        SQLiteDatabase db = getWritableDatabase();
        String query = "SELECT sum("+ tag_col + ")%100000,sum(" + modified_col+")%100000 FROM " + TABLE_NAME;
        Cursor c = db.rawQuery(query, null);
        c.moveToFirst();
...
public void update_Values(Shoplist_item item){
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL("UPDATE " + TABLE_NAME + " set " + name_col + " =  \"" + item.name + "\" , " + deleted_col + " = " + item.deleted + " , " + priority_col + " =  \"" + item.priority+ "\" , " + modified_col + " = " + item.modified + " , " + servid_col + " = " + item.serve_id +  " WHERE " + COLUMN_ID  + " = \"" + item.client_id + "\" ;" );
    }
